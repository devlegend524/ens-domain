...
const Name = function(name, provider) {
  // lowercase the name. if someone passes in NAME.weth
  // that is equivalent to name.weth
  ...
  return {
    ...
    resolve: async key => {
      ...
      let resolver;
      let domain = await getDomain(); // this is the domain with 2 labels, e.g. name.weth
