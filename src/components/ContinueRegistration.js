...
class ContinueRegistration extends React.PureComponent {
  componentDidMount() {
    services.linking.addEventListener(
      services.linking.EVENTS.ROUTE_CHANGED,
      this.handleRouteChange.bind(this)
    );
  }

  componentWillUnmount() {
    services.linking.removeEventListener(
      services.linking.EVENTS.ROUTE_CHANGED,
      this.handleRouteChange.bind(this)
    );
  }
  ...
  render() {
    ...
    if (window.location.pathname === services.linking.path("Register"))
      return null;
    return (
        ...
        >
          <div className="text-xs w-full bg-alert-blue text-white p-2 text-center cursor-pointer">
            <div className="font-bold flex items-center justify-center">
              <span className="w-4" />
              {this.props.cartNames.length === 1
                ? "You have an incomplete domain registration"
                : "You have incomplete domain registrations"}
